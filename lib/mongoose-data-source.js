...
class MongooseDataSource {
  ...
  remove(doc) {
    return 'deleteOne' in doc ? doc.deleteOne() : doc.remove();
  }
  ...
  getModelFieldNames() {
    ...
    return _.filter(paths, (fieldName) => (fieldName === '_id' || fieldName.startsWith('__')));
  }
  ...
  _buildQ(qFields, q) {
    const qExpr = [];
    qFields.forEach(qFields, (qField) => {
      const obj = {};
      try {
        if (qField === '_id') {
          obj[qField] = new ObjectID(q);
        } else {
