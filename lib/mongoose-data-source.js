...
class MongooseDataSource {
  ...
  find(options) {
    ...
    const resolved = this._resolveAssociations(this.ModelClass, fields);

    const populationQuery = resolved.populationQuery;
    ...
    const query = this.ModelClass.find(filter, resolved.fields);
  ...
  findOne(options) {
    ...
    const query = this.ModelClass.findOne(filter, resolved.fields);
  ...
  parseError(err) {
    ...
    if (err.name === 'ValidationError') {
      ...
      result.details = _.reduce(err.errors, (result, { kind, path, value, message }, key) => {
        result[key] = { kind, path, value, message };
        return result;
      }, {});
  ...
  _resolveAssociations(ModelClass, fields) {
